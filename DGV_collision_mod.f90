...
if (momsrates_reliab) then 
 ...
 if (scrtheta(1) < 0) then
    print *, "EvalColRelES scrtheta(1) is negative. Stop"
    stop
 endif
 if (scrtheta(2) < 0) then
    print *, "EvalColRelES scrtheta(2) is negative. Stop"
    stop
 endif
 if (scrtheta(3) < 0) then
    print *, "EvalColRelES scrtheta(3) is negative. Stop"
    stop
 endif
...
select case (run_mode) ! run_mode is set outside, when solution is evaluated for closeness to a maxwellian...  
	...
	case (1) ! this is the non-linear perturbation mode: we need both the linear part and the non-linear 
	  ...
	  !!!!!!!!!!!!!
	  !!  MATH 582D, PROJECT 2  
	  !!  Estimating the time complexity of the two algorithms and error of evaluating zero:
	  !!  Important: Make sure the mode treshlods are such that the solution switches to mode 1 on the first step !
	  !!  (If you are not sure what this means ask your professor)
	  !!!!!!!!!!!!!
      !allocate (fcol_scr(1:size(fcol,1)), stat=loc_alloc_stat)
      !if (loc_alloc_stat >0) then 
	  ...
	  ! Uncomment if using Guass integration of the collision integral 
      !
      ! read time before evaluation 
      ! call CPU_TIME(dtime)
      ! uncomment if using full form
      ! call EvalCollisionPeriodicAPlus_DGV(f,fcol_scr)
      ! Uncomment if evaluating  of collision operator in decomposed form
	  ...
	  ! read time after evaluation
	  ! call CPU_TIME(temptime)
	  ! evaluate the time lapsed
      ...
      ! read the time before evaluation
      !call CPU_TIME(ftime)
      ! uncomment if evaluating of collision operator in full form 
      !call EvalFourierColTwoZeroPadF_OMP(f,f,fcol,pad)
      ! uncomment if evaluating of collision operator in decomposed form 
      !call EvalFourierColTwoZeroPadF_OMP(2*f-Df,Df,fcol,pad)
      ! read the time after evaluation 
      !call CPU_TIME(temptime)
      ! compute time lapsed
      ...
      !! Print report 
      !!
      !print *,"Combined CPU time to evaluate collision operator directly:", dtime
      !print *,"Combined CPU time to evaluate collision operator using FFT:", ftime
      !!
      !!!!!!!!!!!!!!!!!!!!!!!!!
      !!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      !!!!!!!!!!!!!!!!!!!!!!!!!
      !!!! Zeroth moment
      !xxx0 = abs(sum(fcol*nodes_gwts))
      !xxx1 = abs(sum(fcol_scr*nodes_gwts))
      !!!! Report
      !print *,"Error in zeroth moment, collision operator directly:", xxx1
      !print *,"Error in zeroth moment, collision operator using FFT:", xxx0
      !!!! First moment 
      !xxx0 = abs(sum(fcol*nodes_u*nodes_gwts))
      !xxx1 = abs(sum(fcol_scr*nodes_u*nodes_gwts))
      !!!! Report
      !print *,"Error in first moment, collision operator directly:", xxx1
      !print *,"Error in first moment, collision operator using FFT:", xxx0
      !!!! energy moment 
      !xxx0 = abs(sum(fcol*(nodes_u**2+nodes_v**2+nodes_w**2)*nodes_gwts))
      !xxx1 = abs(sum(fcol_scr*(nodes_u**2+nodes_v**2+nodes_w**2)*nodes_gwts))
      !!!! Report
      !print *,"Error in energy, collision operator directly:", xxx1
      !print *,"Error in energy, collision operator using FFT:", xxx0
      !!!
      !!! Difference between 
      
      !print *,"Combined CPU time to evaluate collision operator using FFT:", ftime
      !xxx0 = maxval(abs(fcol-fcol_scr))
      !xxx1 = sum(abs(fcol-fcol_scr)*nodes_gwts)
      !xxx2 = sum(abs(fcol-fcol_scr)**2*nodes_gwts)
	  !! Now we are addiing the dimensionless coefficient mol_diam^2*N_inf/(L_inf)^2 
	  !print *,"Linf err:", xxx0
      !print *,"L1 err:",xxx1
      !print *,"L2 err:",xxx2
      
      !xxx0 = maxval(abs(fcol-fcol_scr))/maxval(abs(fcol))
      !xxx1 = sum(abs(fcol-fcol_scr)*nodes_gwts)/sum(abs(fcol)*nodes_gwts)
      !xxx2 = sum(abs(fcol-fcol_scr)**2*nodes_gwts)/sum(abs(fcol)**2*nodes_gwts)
	  !! Now we are addiing the dimensionless coefficient mol_diam^2*N_inf/(L_inf)^2 
	  coef_temp = (mol_diam/L_inf)**2*N_inf*dt
	  fcol = fcol*coef_temp
